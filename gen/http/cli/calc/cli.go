// Code generated by goa v3.0.2, DO NOT EDIT.
//
// calc HTTP client CLI support package
//
// Command:
// $ goa gen goa-ddb/design

package cli

import (
	"flag"
	"fmt"
	calcc "goa-ddb/gen/http/calc/client"
	"net/http"
	"os"

	goahttp "goa.design/goa/v3/http"
	goa "goa.design/goa/v3/pkg"
)

// UsageCommands returns the set of commands and sub-commands using the format
//
//    command (subcommand1|subcommand2|...)
//
func UsageCommands() string {
	return `calc (add|addresume|list)
`
}

// UsageExamples produces an example of a valid invocation of the CLI tool.
func UsageExamples() string {
	return os.Args[0] + ` calc add --a 4591908534116225627 --b 2205149088366403631` + "\n" +
		""
}

// ParseEndpoint returns the endpoint and payload as specified on the command
// line.
func ParseEndpoint(
	scheme, host string,
	doer goahttp.Doer,
	enc func(*http.Request) goahttp.Encoder,
	dec func(*http.Response) goahttp.Decoder,
	restore bool,
	calcAddresumeEncoderFn calcc.CalcAddresumeEncoderFunc,
) (goa.Endpoint, interface{}, error) {
	var (
		calcFlags = flag.NewFlagSet("calc", flag.ContinueOnError)

		calcAddFlags = flag.NewFlagSet("add", flag.ExitOnError)
		calcAddAFlag = calcAddFlags.String("a", "REQUIRED", "Left operand")
		calcAddBFlag = calcAddFlags.String("b", "REQUIRED", "Right operand")

		calcAddresumeFlags    = flag.NewFlagSet("addresume", flag.ExitOnError)
		calcAddresumeBodyFlag = calcAddresumeFlags.String("body", "REQUIRED", "")

		calcListFlags = flag.NewFlagSet("list", flag.ExitOnError)
	)
	calcFlags.Usage = calcUsage
	calcAddFlags.Usage = calcAddUsage
	calcAddresumeFlags.Usage = calcAddresumeUsage
	calcListFlags.Usage = calcListUsage

	if err := flag.CommandLine.Parse(os.Args[1:]); err != nil {
		return nil, nil, err
	}

	if flag.NArg() < 2 { // two non flag args are required: SERVICE and ENDPOINT (aka COMMAND)
		return nil, nil, fmt.Errorf("not enough arguments")
	}

	var (
		svcn string
		svcf *flag.FlagSet
	)
	{
		svcn = flag.Arg(0)
		switch svcn {
		case "calc":
			svcf = calcFlags
		default:
			return nil, nil, fmt.Errorf("unknown service %q", svcn)
		}
	}
	if err := svcf.Parse(flag.Args()[1:]); err != nil {
		return nil, nil, err
	}

	var (
		epn string
		epf *flag.FlagSet
	)
	{
		epn = svcf.Arg(0)
		switch svcn {
		case "calc":
			switch epn {
			case "add":
				epf = calcAddFlags

			case "addresume":
				epf = calcAddresumeFlags

			case "list":
				epf = calcListFlags

			}

		}
	}
	if epf == nil {
		return nil, nil, fmt.Errorf("unknown %q endpoint %q", svcn, epn)
	}

	// Parse endpoint flags if any
	if svcf.NArg() > 1 {
		if err := epf.Parse(svcf.Args()[1:]); err != nil {
			return nil, nil, err
		}
	}

	var (
		data     interface{}
		endpoint goa.Endpoint
		err      error
	)
	{
		switch svcn {
		case "calc":
			c := calcc.NewClient(scheme, host, doer, enc, dec, restore)
			switch epn {
			case "add":
				endpoint = c.Add()
				data, err = calcc.BuildAddPayload(*calcAddAFlag, *calcAddBFlag)
			case "addresume":
				endpoint = c.Addresume(calcAddresumeEncoderFn)
				data, err = calcc.BuildAddresumePayload(*calcAddresumeBodyFlag)
			case "list":
				endpoint = c.List()
				data = nil
			}
		}
	}
	if err != nil {
		return nil, nil, err
	}

	return endpoint, data, nil
}

// calcUsage displays the usage of the calc command and its subcommands.
func calcUsage() {
	fmt.Fprintf(os.Stderr, `The calc service performs operations on numbers
Usage:
    %s [globalflags] calc COMMAND [flags]

COMMAND:
    add: Add implements add.
    addresume: Addresume implements addresume.
    list: List implements list.

Additional help:
    %s calc COMMAND --help
`, os.Args[0], os.Args[0])
}
func calcAddUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] calc add -a INT -b INT

Add implements add.
    -a INT: Left operand
    -b INT: Right operand

Example:
    `+os.Args[0]+` calc add --a 4591908534116225627 --b 2205149088366403631
`, os.Args[0])
}

func calcAddresumeUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] calc addresume -body JSON

Addresume implements addresume.
    -body JSON: 

Example:
    `+os.Args[0]+` calc addresume --body '[
      {
         "education": [
            {
               "institution": "Ex quae facilis necessitatibus.",
               "major": "Quasi est eum ad."
            },
            {
               "institution": "Ex quae facilis necessitatibus.",
               "major": "Quasi est eum ad."
            }
         ],
         "experience": [
            {
               "company": "Quo est et.",
               "duration": 5058434971892362792,
               "role": "Aut adipisci temporibus laboriosam."
            },
            {
               "company": "Quo est et.",
               "duration": 5058434971892362792,
               "role": "Aut adipisci temporibus laboriosam."
            },
            {
               "company": "Quo est et.",
               "duration": 5058434971892362792,
               "role": "Aut adipisci temporibus laboriosam."
            }
         ],
         "name": "Iusto fugiat sed repudiandae."
      },
      {
         "education": [
            {
               "institution": "Ex quae facilis necessitatibus.",
               "major": "Quasi est eum ad."
            },
            {
               "institution": "Ex quae facilis necessitatibus.",
               "major": "Quasi est eum ad."
            }
         ],
         "experience": [
            {
               "company": "Quo est et.",
               "duration": 5058434971892362792,
               "role": "Aut adipisci temporibus laboriosam."
            },
            {
               "company": "Quo est et.",
               "duration": 5058434971892362792,
               "role": "Aut adipisci temporibus laboriosam."
            },
            {
               "company": "Quo est et.",
               "duration": 5058434971892362792,
               "role": "Aut adipisci temporibus laboriosam."
            }
         ],
         "name": "Iusto fugiat sed repudiandae."
      }
   ]'
`, os.Args[0])
}

func calcListUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] calc list

List implements list.

Example:
    `+os.Args[0]+` calc list
`, os.Args[0])
}
